...
namespace Metrics.Json
{
    public class JsonBuilderV1
    {
        ...
        public JsonBuilderV1 AddObject(IEnumerable<BarGaugeValueSource> bargauges)
        {
            var properties = bargauges.Select(bg => new { Name = bg.Name, Value = bg.Value, YMax = bg.YMax})
                .Select(bg => new JsonProperty(bg.Name, new[]
                {
                    new JsonProperty("Value", bg.Value),
                    new JsonProperty("YMax", bg.YMax),
                }));
            root.Add(new JsonProperty("BarGauges", properties));
            units.Add(new JsonProperty("BarGauges", bargauges.Select(bg => new JsonProperty(bg.Name, bg.Unit.Name))));
            return this;
        }
